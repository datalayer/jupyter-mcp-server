...
"""Regression test for #236: extensions/mcpb/manifest.json and extensions/mcpb/pyproject.toml must
...
def test_manifest_version_matches_package_version():
    manifest = json.loads((ROOT / "extensions" / "mcpb" / "manifest.json").read_text())
    assert manifest["version"] == _package_version()


def test_mcpb_pyproject_version_matches_package_version():
    text = (ROOT / "extensions" / "mcpb" / "pyproject.toml").read_text()
...
def test_find_drift_reports_each_mismatched_file():
    ...
    assert find_drift("1.0.7", "0.22.1", "1.0.7") == [
        "extensions/mcpb/manifest.json is 0.22.1, expected 1.0.7"
    ]
    assert find_drift("1.0.7", "0.22.1", "1.0.0") == [
        "extensions/mcpb/manifest.json is 0.22.1, expected 1.0.7",
        "extensions/mcpb/pyproject.toml is 1.0.0, expected 1.0.7",
    ]
