...
class JupyterMCPConfig(BaseModel):
    ...
    def uses_sandbox_variant(self) -> bool:
        ...
        Read through `code_sandboxes`' own `normalize_variant`, so the answer
        does not depend on whether the value was written with a dash, an
        underscore or in capitals, and so this package and that one always
        agree on which variant a name means.
        """
