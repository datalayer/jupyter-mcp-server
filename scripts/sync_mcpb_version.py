...
"""Sync extensions/mcpb/manifest.json and extensions/mcpb/pyproject.toml's version fields to
...
MANIFEST_FILE = ROOT / "extensions" / "mcpb" / "manifest.json"
PYPROJECT_FILE = ROOT / "extensions" / "mcpb" / "pyproject.toml"
...
def find_drift(package_version: str, manifest_version: str, pyproject_version: str) -> list:
    ...
    if manifest_version != package_version:
        drift.append(f"extensions/mcpb/manifest.json is {manifest_version}, expected {package_version}")
    if pyproject_version != package_version:
        drift.append(f"extensions/mcpb/pyproject.toml is {pyproject_version}, expected {package_version}")
    return drift
...
def main() -> int:
    ...
    if manifest_version != package_version:
        ...
        print(f"extensions/mcpb/manifest.json: {manifest_version} -> {package_version}")
    if pyproject_version != package_version:
        write_pyproject_version(package_version)
        print(f"extensions/mcpb/pyproject.toml: {pyproject_version} -> {package_version}")
    return 0
