...
This binds the listener to loopback, so it is reachable only from the same machine.
To accept connections from other hosts, pass `--host 0.0.0.0` (or set `HOST`), and put
the server behind something that terminates TLS. The Docker image sets `HOST=0.0.0.0`
already, because a published port is forwarded to the container's own interface rather
than to its loopback.
...
For advanced configuration options, see the [server configuration guide](/operations/configuration).
...
For detailed configuration and troubleshooting, see the [configuration guide](/operations/configuration).
