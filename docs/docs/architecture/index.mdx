...
flowchart TB
    ...
    tools["Tool layer<br/>tools/ — 18 tools + jupyter_cite prompt<br/>BaseTool.execute(mode, …)"]
    server --> tools
    handlers --> tools
...
Two Python extensions ship in `ext/`:

- **jupyter-mcp-sandboxes** (`ext/sandboxes`) — the sandbox lifecycle tools
...
- **jupyter-mcp-spaces** (`ext/spaces`) — for the `datalayer` document
  provider: replaces `list_notebooks` with one that lists the notebooks in the
  user's spaces, teaches `use_notebook` to accept a name or a uid, and removes
  the tools that assume a Jupyter server so an agent is never offered a tool
  that cannot work.

`ext/` also holds packaging that is not Python — the MCPB bundle for one-click
installation in Claude Desktop (`ext/mcpb`), a Claude plugin
(`ext/claude-plugin`) and prompt templates (`ext/prompt-templates`).
...
├── spaces/                  jupyter-mcp-spaces extension
