...
:::note A change by somebody else *is* covered, by a watcher

This section describes what *this worker* changed, and for a while that was
the whole story: notebook connections were per call, so between calls there
was no live document to observe and a person typing in JupyterLab was
invisible.
