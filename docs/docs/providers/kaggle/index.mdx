...
variant (backed by `jupyter-kernel-client`'s `KaggleKernelClient`).
...
1. **Create a new kernel** on the runtime proxy URL using a Kaggle API token.
2. **Connect to an existing kernel** using either `RUNTIME_ID` or a
   `RUNTIME_CHANNELS_URL` to derive runtime details.

### Mode A: create a new kernel (token-based)
...
Select the Kaggle engine with `SANDBOX_VARIANT=kaggle`:
