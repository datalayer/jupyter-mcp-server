...
MCP clients must authenticate to the MCP server by sending an `Authorization` header with `MCP_TOKEN` or,  if `MCP_TOKEN` was not set, the `RUNTIME_TOKEN`.
...
### Using a Dedicated MCP Token

To use a separate token for MCP client authentication, set `MCP_TOKEN` when starting the server:

```bash
jupyter-mcp-server start \
  --transport streamable-http \
  --jupyter-url http://localhost:8888 \
  --jupyter-token MY_JUPYTER_TOKEN \
  --mcp-token MY_MCP_TOKEN \
  --port 4040
```

Then configure your MCP client with `MY_MCP_TOKEN` instead of the Jupyter token.
