...
From 1.4.5, `jupyter-mcp-server` requires `code-sandboxes >= 1.1.1`, so pip refuses that
...
If you see that, you are on `jupyter-mcp-server` < 1.4.5 with `code-sandboxes` >= 1.1.1.
...
1.4.5 `jupyter-mcp-server` uses those names, and `SANDBOX_VARIANT=jupyter-server` is the
default.

Spelling is read leniently from 1.4.5 onward — `google_colab`, `google-colab` and
