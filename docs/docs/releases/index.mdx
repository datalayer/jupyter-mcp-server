...
`code-sandboxes` 1.1.0, so the two packages have to agree on the name. Which version of
...
From 1.4.5, `jupyter-mcp-server` requires `code-sandboxes >= 1.1.0`, so pip refuses that
pairing outright. The one it does **not** catch is the other direction: earlier releases
asked only for `code-sandboxes >= 0.17.0`, which a 1.1.0 install satisfies. Nothing fails
...
If you see that, you are on `jupyter-mcp-server` < 1.4.5 with `code-sandboxes` >= 1.1.0.
...
`code-sandboxes` 1.1.0 made every canonical variant name spell out what it is:
