...
| `SANDBOX_VARIANT` | Code sandbox variant (see [Sandbox Variants](#sandbox-variants)) | `jupyter-server` / `kaggle` / `monty` / `google-colab` / `modal` / `docker` / `eval` / `daytona` / `datalayer` | `jupyter-server` |
...
Options:
  ...
  --sandbox-variant TEXT             Code sandbox variant: jupyter-server (default), kaggle, google-colab, monty, modal, docker, eval, daytona, datalayer
