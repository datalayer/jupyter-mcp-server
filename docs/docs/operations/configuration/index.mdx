...
| `SANDBOX_VARIANT` | Code sandbox variant (see [Sandbox Variants](#sandbox-variants)) | `jupyter-server` / `kaggle` / `monty` / `google-colab` / `modal` / `docker` / `eval` / `daytona` / `e2b` / `coreweave` / `cloudflare` / `datalayer` | `jupyter-server` |
| `CODE_SANDBOX_PROXY_TOKEN` | Proxy token used by the `google-colab` sandbox variant (`colab-runtime-proxy-token`) | `ya29...` | `None` |
| `CODE_SANDBOX_CHANNELS_URL` | For the `kaggle` and `google-colab` engines, notebook session websocket channels URL used to derive `CODE_SANDBOX_URL` and `CODE_SANDBOX_ID` | `wss://.../api/kernels/.../channels?...` | `None` |
| `SANDBOX_ENVIRONMENT` | Environment name for cloud sandboxes (Datalayer/Modal) | `python-cpu-env` | `None` |
| `SANDBOX_GPU` | GPU flavor / accelerator. Only `coreweave`, `datalayer`, `daytona`, `kaggle` and `modal` have a GPU; setting it for a variant that has none is refused when the sandbox starts, rather than quietly running on a CPU, so leave it unset for those. Modal/Datalayer examples: `T4`, `A10G`, `A100`, `H100`. Daytona examples: `H100`, `H200`, `RTX-4090`. CoreWeave example: `H100`. Kaggle batch examples: `NvidiaTeslaT4`, `NvidiaTeslaP100` (aliases `T4`, `P100` also supported). | `T4` | `None` |

## Sandbox Variants

By default the server executes code through the `code-sandboxes`
`jupyter-server` engine against a Jupyter Server
(`SANDBOX_VARIANT=jupyter-server`). Setting
...
[`jupyter_mcp_sandboxes`](https://github.com/datalayer/jupyter-mcp-server/tree/main/extensions/sandboxes)
extension. Install it (`pip install jupyter_mcp_sandboxes`) to expose the sandbox
lifecycle tools and use any non-`jupyter-server` sandbox variant.
...
| `google-colab` | Google Colab code sandbox | `jupyter-mcp-server[google-colab]` (or base install) | `CODE_SANDBOX_URL`, `CODE_SANDBOX_ID`, `CODE_SANDBOX_PROXY_TOKEN` |
...
| `daytona` | [Daytona](https://www.daytona.io/docs/) cloud sandbox | `jupyter-mcp-server[daytona]` | `DAYTONA_API_KEY`, or `DAYTONA_JWT_TOKEN` + `DAYTONA_ORGANIZATION_ID`. Optional accelerator: `SANDBOX_GPU`. |
| `e2b` | [E2B](https://docs.e2b.dev/client) sandbox | `jupyter-mcp-server[e2b]` | `E2B_API_KEY`. Optional: `E2B_DOMAIN`. |
| `coreweave` | [CoreWeave](https://docs.coreweave.com/products/sandboxes) sandbox | `jupyter-mcp-server[coreweave]` | `CWSANDBOX_API_KEY`. Optional: `CWSANDBOX_BASE_URL`, accelerator `SANDBOX_GPU`. |
| `cloudflare` | [Cloudflare](https://developers.cloudflare.com/sandbox/) sandbox, reached through the sandbox bridge Worker | `jupyter-mcp-server[cloudflare]` | `CLOUDFLARE_SANDBOX_API_URL`, `CLOUDFLARE_SANDBOX_API_KEY` |
...
### Example: Daytona engine

```bash
SANDBOX_VARIANT=daytona
DAYTONA_API_KEY=your-daytona-api-key
# ...or a JWT token, which is read with the organization it belongs to:
...
# E2B_DOMAIN only when the deployment is not the default e2b.dev:
# E2B_DOMAIN=your-e2b-domain
```

### Example: CoreWeave engine

```bash
SANDBOX_VARIANT=coreweave
CWSANDBOX_API_KEY=your-coreweave-api-key
SANDBOX_GPU=H100
# CWSANDBOX_BASE_URL only when the endpoint is not the default https://api.cwsandbox.com:
# CWSANDBOX_BASE_URL=https://api.cwsandbox.com
```

### Example: Cloudflare engine

Cloudflare's SDK is a TypeScript Workers binding, which a Python process cannot
hold, so this engine talks to the sandbox bridge — a reference Worker Cloudflare
publishes that exposes the SDK over HTTP. Deploy it once:
...
Options:
  ...
  --sandbox-variant TEXT             Code sandbox variant: jupyter-server (default), kaggle, google-colab, monty, modal, docker, eval, daytona, e2b, coreweave, cloudflare, datalayer
  --code-sandbox-proxy-token TEXT          Proxy token for the google-colab sandbox variant (default: None)
  --code-sandbox-channels-url TEXT         WebSocket channels URL for the kaggle or google-colab sandbox variants (default: None)
