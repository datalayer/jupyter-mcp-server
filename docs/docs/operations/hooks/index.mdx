...
Jupyter MCP Server includes a **hook system** that fires events before and after MCP tool calls and kernel executions. This enables auditing and custom integrations without modifying core server code. For built-in tracing, see [Observability](/operations/observability).
