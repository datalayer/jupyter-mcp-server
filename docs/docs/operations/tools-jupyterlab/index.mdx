...
When running in JupyterLab mode (see [Configuration](/operations/configuration)), Jupyter MCP Server depends on **jupyter-mcp-tools** to provide enhanced UI integration capabilities. This extension enables the use of JupyterLab commands as MCP tools.
...
Answering `tools/list` waits for the JupyterLab frontend (`jupyter-mcp-tools`) to finish registering its tools. Registration walks every installed JupyterLab command, so on deployments with many extensions it can take far longer than the default wait of 5 seconds (44 seconds measured with 585 registered commands, see [#470](https://github.com/datalayer/jupyter-mcp-server/issues/470)). When the wait expires, the session silently ends up with zero frontend tools.

If your deployment ships many JupyterLab extensions, raise the wait:

```bash
jupyter lab --port 4040 --IdentityProvider.token MY_TOKEN --JupyterMCPServerExtensionApp.jupyter_mcp_tools_timeout=60
```

The default stays at 5 seconds so a missing frontend still fails fast.
