...
- Input:
  ...
  - `variant`(string, optional): Sandbox variant (`eval`, `docker`, `jupyter`, `datalayer`, `colab`, `kaggle`, `monty`, `modal`, default: `eval`)
  ...
  - `server_url`(string, optional): Runtime proxy URL (colab or kaggle variant)
  - `kernel_id`(string, optional): Kernel ID (colab or kaggle variant; omit for kaggle to create a new kernel)
  - `channels_url`(string, optional): Kaggle notebook session channels URL to derive `server_url`/`kernel_id` (kaggle variant)
