...
## Server and Code Sandbox Management (3 + 4 optional tools)
...
- Launch a code sandbox that can be used as an alternative execution backend to Jupyter kernels.
- Input:
  ...
  - `environment`(string, optional): Code sandbox environment name (common for datalayer/modal)
  - `gpu`(string, optional): GPU flavor for supported variants (for example `T4`, `A10G`, `A100`, `H100`)
  - `server_url`(string, optional): Code Sandbox proxy URL (kaggle or colab variant)
  ...
  - `proxy_token`(string, optional): Colab code sandbox proxy token (colab variant)
...
- List all launched code sandboxes and their status.
- Input: None
...
- Terminate and unregister a launched code sandbox.
- Input:
