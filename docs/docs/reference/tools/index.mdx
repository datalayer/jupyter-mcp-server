...
The server offers 18 tools by default, plus 4 sandbox tools when sandbox features are enabled with `--enable-sandboxes` (or `ENABLE_SANDBOXES=true`).
...
This tool is hidden unless sandbox features are enabled with `--enable-sandboxes` (or `ENABLE_SANDBOXES=true`).
:::
...
- Input:
  ...
  - `variant`(string, optional): Sandbox variant (`eval`, `docker`, `jupyter`, `datalayer`, `colab`, `monty`, `modal`, default: `eval`)
  ...
  - `server_url`(string, optional): Colab runtime URL (colab variant)
  - `kernel_id`(string, optional): Colab kernel ID (colab variant)
  - `proxy_token`(string, optional): Colab runtime proxy token (colab variant)
  - `use_browser_bridge`(bool, optional): For colab variant, derive runtime details from authenticated browser session (default: `false`)
  - `token`(string, optional): Datalayer API token override
...
:::note Feature flag required for sandbox routing
Sandbox routing in `execute_code` only applies when sandbox features are enabled with `--enable-sandboxes` (or `ENABLE_SANDBOXES=true`).
:::

#### 22. `clear_cell_output`
