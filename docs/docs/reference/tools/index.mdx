...
- List all available kernel specs and active running kernels in the Jupyter server.
- This tool lists both available kernel specs (templates for creating new kernels) and currently active running kernels on the Jupyter server, formatted into two separate TSV sections. Useful for checking available kernel specs and monitoring active kernel resources.
- Input: None
- Returns: Two tab-separated tables formatted as follows:
  - **Section 1: Available Kernel Specs**
    - **Name (kernel_name)**: Kernel spec name/identifier
  ...
  - **Section 2: Active Running Kernels**
    - **ID (kernel_id)**: Unique active kernel identifier
    - **Spec_Name**: Kernel spec name used by the active kernel
...
- Input:
  ...
  - `kernel_spec_name`(string, optional): Kernel spec name to use when starting a new kernel (default: "python3")
