...
*Either `JUPYTER_TOKEN` or `JUPYTER_PASSWORD` is required when connecting to secured Jupyter servers. See [Security](/reference/security) for details on when to use each.
