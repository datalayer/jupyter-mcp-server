...
| `EXECUTION_ENGINE` | Code execution engine (see [Execution Engines](#execution-engines)) | `jupyter` / `colab` / `monty` / `modal` / `docker` / `eval` / `datalayer` | `jupyter` |
| `RUNTIME_PROXY_TOKEN` | Proxy token used by the `colab` execution engine (`colab-runtime-proxy-token`) | `ya29...` | `None` |
...
Jupyter Server (`EXECUTION_ENGINE=jupyter`). Setting `EXECUTION_ENGINE` to any
...
| Engine | Description | Extra install | Key variables |
|--------|-------------|---------------|---------------|
...
Options:
  ...
  --execution-engine TEXT             Code execution engine: jupyter (default), colab, monty, modal, docker, eval, datalayer
  --runtime-proxy-token TEXT          Proxy token for the colab execution engine (default: None)
