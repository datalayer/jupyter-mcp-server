...
| `SANDBOX_VARIANT` | Code sandbox variant (see [Sandbox Variants](#sandbox-variants)) | `jupyter` / `colab` / `monty` / `modal` / `docker` / `eval` / `datalayer` | `jupyter` |
| `ENABLE_SANDBOXES` | Enable sandbox lifecycle tools and sandbox-backed execution routes | `true` / `false` | `false` |
...
Sandbox functionality is opt-in. To expose sandbox lifecycle tools and use any
non-`jupyter` sandbox variant, set `ENABLE_SANDBOXES=true` (or start with
`--enable-sandboxes`).
...
Options:
  ...
  --sandbox-variant TEXT             Code sandbox variant: jupyter (default), colab, monty, modal, docker, eval, datalayer
  --enable-sandboxes / --disable-sandboxes
                                      Enable sandbox lifecycle tools and sandbox-backed execution routes (default: disabled)
