...
| Variable | Description | Example | Default |
|----------|-------------|---------|---------|
| `MCP_TOKEN` | Token for authenticating MCP clients (Bearer scheme). | `mcp-client-secret` | Value of `RUNTIME_TOKEN` |
...
Options:
  ...
  --mcp-token TEXT                    Token for authenticating MCP clients; falls back to RUNTIME_TOKEN
