...
| `SANDBOX_VARIANT` | Code sandbox variant (see [Sandbox Variants](#sandbox-variants)) | `jupyter` / `colab` / `kaggle` / `monty` / `modal` / `docker` / `eval` / `datalayer` | `jupyter` |
| `RUNTIME_PROXY_TOKEN` | Proxy token used by the `colab` sandbox variant (`colab-runtime-proxy-token`) | `ya29...` | `None` |
| `RUNTIME_USE_BROWSER_BRIDGE` | For the `colab` engine, obtain the runtime connection details from an authenticated Colab browser session via `jupyter-kernel-client`'s browser bridge | `true` / `false` | `false` |
| `RUNTIME_CHANNELS_URL` | For the `kaggle` engine, notebook session websocket channels URL used to derive `RUNTIME_URL` and `RUNTIME_ID` | `wss://.../proxy/api/kernels/.../channels?...` | `None` |
...
| `kaggle` | Kaggle notebook runtime | `jupyter-mcp-server[kaggle]` | `RUNTIME_URL` (+ `KAGGLE_API_TOKEN`, or `RUNTIME_ID`) |
...
# Create a new kernel with a Kaggle API token:
KAGGLE_API_TOKEN=...
...
Options:
  ...
  --sandbox-variant TEXT             Code sandbox variant: jupyter (default), colab, kaggle, monty, modal, docker, eval, datalayer
  --runtime-proxy-token TEXT          Proxy token for the colab sandbox variant (default: None)
  --runtime-channels-url TEXT         WebSocket channels URL for the kaggle sandbox variant (default: None)
