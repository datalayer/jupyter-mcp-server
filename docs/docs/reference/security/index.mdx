...
The standalone MCP server can be explicitly configured with `MCP_TOKEN` credentials, independently from `RUNTIME_TOKEN` it uses to authenticate with Jupyter.
...
When `MCP_TOKEN` is set, only that token is accepted for MCP client requests.
When it is not set, `RUNTIME_TOKEN` is accepted as a fallback.
