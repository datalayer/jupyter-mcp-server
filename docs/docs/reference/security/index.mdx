...
This stores a hashed password in `~/.jupyter/jupyter_server_config.json`. Then start Jupyter without a token:
...
### Alternatives for Token-less Environments

If password auth doesn't fit your deployment:

#### Option 1: Start Jupyter with a Token
