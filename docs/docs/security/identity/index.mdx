...
The built-in authentication described in [Authentication](/security/authentication) covers
...
accounts and your notion of what a client is allowed to do. For the OAuth 2.1
resource-server flow, see [OAuth 2.1](/security/oauth).
...
```python
from mcp.server.auth.provider import AccessToken
from jupyter_mcp_server.identity import FORWARD_CLAIM
...
class MyVerifier:
  ...
  async def verify_token(self, token: str) -> AccessToken | None:
    ...
    return AccessToken(
        ...
        claims={FORWARD_CLAIM: True},  # Opt-in: mark token for upstream forwarding
    )
```

Nothing else is needed. `identity_from_access_token()` copies the bearer token
onto the `Identity` when `FORWARD_CLAIM` (`"jupyter_mcp_forward_token"`) is set to `True`
in `claims`. By default (when `FORWARD_CLAIM` is omitted), bearer tokens are not
forwarded upstream, ensuring verifiers such as `--mcp-token` remain safe by construction.

The configuration resolves it:

```python
config.resolved_document_token()  # the caller's (if marked for forwarding), else the configured one
...
:::warning Serving several users means one process each
...
So the supported way to serve several users is **one server process per user**,
with authentication and authorization in front of them. A process is a boundary
the operating system already enforces, and it covers the state you have not
thought of as well as the state you have.

The hosted [Datalayer](https://datalayer.ai) gateway is built this way: it
verifies the token, checks the scope and the notebook permission, and then
hands the request to that user's own process, which holds no credential of its
own and authenticates the forwarded header exactly as a directly connected
client would.

Per-request credentials still matter inside that design — a worker serves one
...
OAuth access tokens use the same verifier and `Identity` mechanisms described
above. See [OAuth 2.1](/security/oauth) for protected-resource discovery,
audience validation, scope mapping, and the hosted Datalayer example.
