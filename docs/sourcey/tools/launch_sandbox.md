...
| `variant` | `eval` · `docker` · `jupyter-server` · `datalayer` · `daytona` · `google-colab` · `kaggle` · `monty` · `modal` \| null | no | `null` | Sandbox variant to launch. If omitted, defaults to configured SANDBOX_VARIANT when it is not jupyter-server; otherwise falls back to eval. |
